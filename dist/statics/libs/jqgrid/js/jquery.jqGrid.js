...
(function( factory ) {
	...
	if ( typeof define === "function" && define.amd ) {
		...
		define(["grid.locale-cn"
		], function(){factory.apply(this, [jQuery].concat(Array.prototype.slice.call(arguments)))} );
	} else {
		// Browser globals
		factory( jQuery );
	}
		...
		return this.each( function() {
			...
			var sortkeys = ["shiftKey","altKey","ctrlKey"],
				...
				setPager = function (pgid, tp){
					...
					if(ts.p.rowList && ts.p.rowList.length >0){
						str = "<td dir=\""+dir+"\">";
...
//============================================================================
	...
	$.extend($.jgrid.defaults, {
		// mtype: "POST", // 相当于 ajaxGridOptions : {type: "POST"}
		ajaxGridOptions : {
			type: "POST",
	...
	/* jqgrid 格式化配置 start */
	$.extend(true, $.jgrid.regional["cn"].formatter, {
		integer : {defaultValue: ''},
		number : {defaultValue: ''},
		currency : {prefix: "¥", defaultValue: ''}
	});
	$.fn.fmatter || ($.fn.fmatter = {});
	$.extend($.fn.fmatter, {
		datetime : function(data){
			return data ? window.M.Date(data).toString() : "";
		}
	});
	/* jqgrid 格式化配置 end */
	...
	(function(){
		var jqGrid = $.fn.jqGrid,
			n_jqGrid = function(o){
				if($.isPlainObject(o)){
					o.colModel && ($.each(o.colModel, function(i, item){
		...
		$.fn.jqGrid = $.extend(n_jqGrid, jqGrid);
	}());
